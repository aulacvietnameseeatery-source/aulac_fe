...
export const AssignTableModal = ({ reservation, onClose, onSuccess }: AssignTableModalProps) => {
    ...
    return (
        <Dialog open={true} onClose={onClose} title={`${t("title")}: ${reservation.customerName}`} width="550px">
            <form onSubmit={handleAssign} className="p-5 flex flex-col max-h-[80vh]">
                ...
                <div className="flex-1 overflow-y-auto pr-2 min-h-[250px]">
                    <h4 className="text-sm font-semibold text-gray-700 mb-3 flex items-center gap-2">
                        <Armchair size={16} /> {t("assignTable")}
                    </h4>

                    {isLoadingTables ? (
                        <div className="flex justify-center py-10"><Loader2 className="animate-spin text-gray-400 w-8 h-8" /></div>
                    ) : availableTables.length === 0 ? (
                        <p className="text-red-500 text-sm text-center py-8">{t("noTablesAvailable")}</p>
                    ) : (
                        <div className="grid grid-cols-2 gap-3">
                            {availableTables.map(table => {
                                ...
                                return (
                                    <label
                                        key={table.tableId}
                                        className={`flex items-start gap-3 p-3 border rounded-lg transition-all ${isDisabled ? 'opacity-40 cursor-not-allowed bg-gray-50' : 'cursor-pointer'
                ...
                <div className="mt-6 flex justify-between items-center pt-4 border-t border-gray-100">
                    ...
                    <div className="flex gap-2">
                        ...
                        <Button type="submit" disabled={isSubmitting || selectedTableIds.length === 0} className="bg-green-600 hover:bg-green-700">
                            {isSubmitting ? <Loader2 className="w-4 h-4 animate-spin mr-2" /> : null} {t("confirm")}
                        </Button>
                    </div>
                </div>
            </form>
